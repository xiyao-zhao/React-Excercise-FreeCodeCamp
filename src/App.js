...
// 38). Use a ternary expression for conditional rendering
const inputStyle = {
  width: 235,
  margin: 5
};
...
// 39). Render conditionally from props
...
// 40). Change inline CSS conditionally based on component state
...
// 41). Use Array.map() to dynamically render elements
...
// 42). Give sibling elements a unique key attribute
...
function Frameworks() {
  const renderFrameworks = frontEndFrameworks.map(a => <li key={a}>{a}</li>); 
  return (
    <div>
...
// 43). Use Array.filter() to dynamically filter an array
